...
tape('encodes + decodes + offset', function (t) {
  var a = [{ host: '127.0.0.1', port: 80 }]
  var b = [{ host: '127.0.0.1', port: 80 }, { host: '127.0.0.1', port: 8080 }]
  t.same(peers.decode(peers.encode(a, Buffer.alloc(8), 2), 2), a)
  t.same(peers.decode(peers.encode(b, Buffer.alloc(14), 2), 2), b)
  t.end()
})
...
tape('encodes + decodes + offset + end', function (t) {
  var a = [{ host: '127.0.0.1', port: 80 }]
  var b = [{ host: '127.0.0.1', port: 80 }, { host: '127.0.0.1', port: 8080 }]
  t.same(peers.decode(peers.encode(a, Buffer.alloc(100)), 0, 6), a)
  t.same(peers.decode(peers.encode(b, Buffer.alloc(100)), 0, 12), b)
  t.end()
})
...
tape('encodes with peer id', function (t) {
  ...
  var a = [{ id: Buffer.from('hello'), host: '127.0.0.1', port: 80 }]
  var b = [{ id: Buffer.from('hello'), host: '127.0.0.1', port: 80 }, { id: Buffer.from('world'), host: '127.0.0.1', port: 8080 }]

  t.same(p.decode(p.encode(a, Buffer.alloc(100)), 0, 11), a)
  t.same(p.decode(p.encode(b, Buffer.alloc(100)), 0, 22), b)
  t.end()
})
